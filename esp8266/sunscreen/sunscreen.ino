...
#define RX_DATA 0
#define RX_RECEIVE_ENABLE 3
#define LED_PIN 2
#define KAKUPIN LED_PIN
...
void setup()
{
  ...
  pinMode(KAKUPIN, OUTPUT);
  pinMode(RX_RECEIVE_ENABLE, FUNCTION_3);
  pinMode(RX_RECEIVE_ENABLE, OUTPUT);
  pinMode(RX_DATA, INPUT);

  digitalWrite(RX_RECEIVE_ENABLE, LOW);

  Serial.printf("Connecting to %s ", SSID_1);
  //    WiFi.mode(WIFI_STA);

  // Configures static IP address - defined in super secret settings header file
...
{ // send the right file to the client (if it exists)
  ...
  if (path.endsWith("/"))
    path += "index.html";                  // If a folder is requested, send the index file of that folder
